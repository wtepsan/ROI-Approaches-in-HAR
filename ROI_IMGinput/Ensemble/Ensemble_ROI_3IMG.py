...
if __name__ == '__main__':
    if protocol == 'xsub':
        ...
        pickle1 = open(xsub_pickle_fullbody, 'rb')
        pickle2 = open(xsub_pickle_roi7, 'rb')
    else:
        print("XVIEW XVIEW XVIEW XVIEW XVIEW XVIEW XVIEW XVIEW XVIEW")
        PICKLELIST = XVIEW_PICKLE_LIST
        pickle1 = open(xview_pickle_fullbody, 'rb')
        pickle2 = open(xview_pickle_roi7, 'rb')
    ...
    for pickle3_location in PICKLELIST: 
        print(pickle3_location)

        pickle3 = open(pickle3_location, 'rb')
        prediction3 = pickle.load(pickle3)
        _, prediction3= pickle2dict(prediction3) 
        ...
        for dataname in tqdm(datanames): 
            ...
            roi_prediction3 = prediction3[dataname]

            ensemble_sum = roi_prediction+roiXopticalflow_prediction+roi_prediction3
            ensemble_sum_prediction = np.argmax(ensemble_sum)
            ensemble_sum_total_correct += int(ensemble_sum_prediction == int(l))

            ensemble_multiply = (roi_prediction * roiXopticalflow_prediction) * roi_prediction3
